...
export const useSettings = defineStore('settings', () => {
  ...
  const useRecognition = ref(saved.useRecognition)
  const showEnglish = ref(saved.showEnglish)

  const slowRate = computed(() => Math.max(0.5, speechRate.value - 0.3))

  watch([speechRate, useRecognition, showEnglish], () => {
    save<SettingsState>('settings', {
      speechRate: speechRate.value,
      useRecognition: useRecognition.value,
      showEnglish: showEnglish.value,
    })
  })

  return { speechRate, useRecognition, showEnglish, slowRate }
})
